...
**Popularity trend (data caveat):** default is `stable` — only flag `decline` with real evidence. Reliable per-year social data is scarce. What's actually obtainable: **year-over-year vendor counts in news** (most reliable) and **news/coverage tone**; weak supplements are **Google Trends** search-interest (noisy for niche local terms) and **archived follower snapshots** (Wayback/Social Blade rarely index small accounts). True attendance and post-engagement history usually require the organizer's own data. _Example: a "Market Noel declined in 2025" perception was checked and **not supported** — vendor count held at 100+, footprint unchanged, coverage steady, 2026 moving up to the larger BJCC → scored `stable`._

**Schedule fit** = day-window × **time-efficiency**. Day-window: `weekend` or `evening_ok` (weekday after 3pm) = 1.0; `mixed_partial` 0.85; `weekday_daytime` 0.45. **Time-efficiency** = **effective hours** → ≤6h 1.0 · ≤10h 0.97 · ≤16h 0.90 · ≤24h 0.82 · >24h 0.72. Effective hours = on-site (`hours_per_day × min_days`) **+ round-trip drive from Birmingham** (`drive_rt_hrs`: Birmingham 0 · Huntsville ~3 · Chattanooga ~4 · Atlanta ~5 · Nashville ~6 · Franklin ~6.5). This is the **$/hour** lever: a 6-hr local single day beats both a 3-day show and a far-drive market — e.g., The Chattanooga Market's 5 on-site hrs + 4 drive = 9 effective (×0.97), and a Nashville show at 14 on-site + 6 drive = 20 (×0.82). **Use the one-day option** for multi-day markets — set `min_days` to the smallest commitment (e.g., Market Noel's 1-day Fri/Sat booth → min_days = 1, 8 hrs). If `hours_per_day` is blank, falls back to a days-only penalty (≤2 days 1.0; 3 → 0.93; 4 → 0.85; 5+ → 0.78).
